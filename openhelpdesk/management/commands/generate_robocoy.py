        ...
        try:
            ...
            for ente, domain in DATA_ENTI:
                ...
                result = con.search_s(dn_ente, ldap.SCOPE_SUBTREE,
                                      # criteria)
                                      criteria, attributes)
                excludes = ['zpbiblioteca', 'zolaadminpec', 'zolaadmindocer',
                            'sistemiinformatici', 'linuxclient']
                users = list(sorted([lu[1]['sAMAccountName'][0].decode("utf-8")
                                     for lu in result]))
                users = [(u, u[1:] + '_' + u[0]) for u in users if u not in excludes]
                # zoladom_users = [s[1:] + '_' + s[0] for s in users]

                # print(list(zip(users, zoladom_users)))
                for user, z_user in users:
                    out = "robocopy %SOURCE%\{} %DEST%\\{} %OPTIONS%" \
                          " /log:%LOGS%\\{}_%STARTTIME%.log".format(
                            z_user, user, user)
                    print(out)
